...
def main():
    ...
    ax1.plot(hdu.data["WAVE"], hdu.data["G"], colors[0] + ltype, label="Total")
    ...
    ax1.set_ylabel(r"$g$", fontsize=fontsize)
    ax1.legend()
    ax1.set_xlim(get_krange(hdu.data["WAVE"], logaxis=True))
    ax1.set_ylim([0.0, 1.0])

    residuals = (hdu.data["G"] - OD.scat_g) / OD.scat_g
    unc = OD.scat_g_unc / OD.scat_g
