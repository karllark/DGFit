...
def main():
    ...
    hdu = hdulist["EXTINCTION"]

    # get the observed data
    OD = ObsData(args.obsfile)

    ax1.plot(hdu.data["WAVE"], hdu.data["EXT"], colors[0] + ltype, label="Total")
    ...
    ax1.errorbar(OD.ext_waves, OD.ext_alav, yerr=OD.ext_alav_unc, fmt='ko', label="Observed", capsize=4)
    yrange = get_krange(OD.ext_alav, logaxis=True, in_range=yrange)

    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ax1.legend()
    ax1.set_xlabel(r"$\lambda [\mu m]$", fontsize=fontsize)
    ax1.set_ylabel(r"$A(\lambda)/A(V)$", fontsize=fontsize)
