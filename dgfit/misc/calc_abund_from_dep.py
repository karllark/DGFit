...
def abundances_from_depletions(depletions, depletions_unc, ref_abundances="SMC"):
    """
    ...
    tot_abund = np.power(10.0, tot_abund_log - 6.0)
    tot_abund_up = np.power(10.0, tot_abund_log + tot_abund_unc_log - 6.0)
    tot_abund_down = np.power(10.0, tot_abund_log - tot_abund_unc_log - 6.0)
    ...
    gas_abund_down = tot_abund * np.power(10.0, gas_abund_log - gas_abund_unc_log)
...
if __name__ == "__main__":
    ...
    results = abundances_from_depletions(
        dep_smc_azv215, dep_unc_smc_azv215, ref_abundances="SMC"
    )
    ...
    a = Table(
        [atomnames, results[0], results[1], results[2], results[3]],
        names=("atom", "abund", "abund_unc", "total_abund", "total_abund_unc"),
    )
